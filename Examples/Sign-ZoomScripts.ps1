...
# Step 2: Define scripts to sign
$scriptRoot = "C:\code\SCCM\Apps\.Net\Core\2-DiscoveryScripts"
# $scriptRoot = "C:\Code\Modulesv2\AsSystem-Module\AsSystem"
Write-SigningLog "Identifying scripts to sign in $scriptRoot"

# Get all PowerShell scripts (.ps1 files) from Scripts and PSADT directories
$psScripts = @(
    # Get-ChildItem -Path "$scriptRoot\Scripts" -Filter "*.ps1" -Recurse
    # Get-ChildItem -Path "$scriptRoot\PSADT" -Filter "*.ps1" -Recurse
    # Get-ChildItem -Path "$scriptRoot\PSADT-CleanZoom" -Filter "*.ps1" -Recurse
...
# Get all PowerShell modules (.psm1 files) from Scripts, PSADT, and Modules directories
$psModules = @(
    # Get-ChildItem -Path "$scriptRoot" -Filter "*.psm1" -Recurse
    # Get-ChildItem -Path "$scriptRoot\PSADT" -Filter "*.psm1" -Recurse
    # Get-ChildItem -Path "$scriptRoot\PSADT-CleanZoom" -Filter "*.psm1" -Recurse
...
try {
    ...
    Write-SigningLog "Loading certificate: $pfxPath" "INFO"
    Write-SigningLog "Certificate requires password, prompting for secure input..." "INFO"
